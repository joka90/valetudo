...
const WebServer = function (valetudo) {
	...
	this.app.get("/api/os/stat", function (req, res) {
	...
	this.app.put("/api/interface_config", function (req, res) {
		...
		['zonedImmediate','gotoImmediate','showMultimap','hideMapStatus','staticMapButtons','hideSegmentMarkers','hideControlMap', 'robotrockButtons'].forEach(option => {
			if (req.body[option]) {
				config[option] = true;
			}
		});
		if(req.body.robotrockCmd) {
			config.robotrockCmd = req.body.robotrockCmd;
		}
	...
	this.app.put("/api/fwd_req", function (req, res) {
		if (req.body && req.body.method !== undefined && req.body.args !== undefined) {
			self.vacuum.fwdReq(req.body.method, req.body.args, function (err, data) {
				if (err) {
					res.status(500).send(err.toString());
				} else {
					res.json(data);
	...
	this.app.put("/api/start_robotrock", function (req, res) {
		if (!this.rockProcess) {
			const web_if_config = self.configuration.get("webInterface");
			if (!web_if_config) {
				res.status(500).send('No webInterface cfg :(.');
				return;
			}
			const rockcmd = web_if_config.robotrockCmd;
			if (!rockcmd) {
				res.status(500).send('No cmd to rock:(.');
				return;
			}
			const rock = spawn('sh', ['-c',  rockcmd]);

			rock.stdout.on('data', (data) => {
				console.log(data.toString());
			});

			rock.stderr.on('data', (data) => {
				console.error(data.toString());
			});

			rock.on('exit', (code) => {
				console.log(`RobotRock ${code}`);
			});
			this.rockProcess = rock;
			res.json({message: "ok"});
		} else {
	...
	this.app.put("/api/stop_robotrock", function (req, res) {
		if (this.rockProcess) {
			this.rockProcess.kill('SIGHUP');
			this.rockProcess = null;
			res.json({message: "ok"});
		} else {
			res.status(500).send('Not started yet.');
		}
	});
